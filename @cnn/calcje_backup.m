...
%    je - gradient (or Jacobian times error), M x 1
%
% where M is the number of trainable parameters (i.e. the size of the
% network)
    ...
    elseif index_cLayer == 1
         ...
         for l=1:cnet.CLayer{k}.numFMaps
                ...
                for m=find(cnet.CLayer{k}.ConMap(l,:)) 
                    ...
                    %fprintf('Calling back_conv2 with following params:\n');
                    %disp(size(cnet.SLayer{k-1}.XS{m}));
                    %disp(size(cnet.CLayer{k}.dEdY{l}));
                    %disp(size(cnet.CLayer{k}.WC{l}));
