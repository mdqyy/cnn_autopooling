...
%    inp - a cell array of input images, 1 x N. Each input image is 
%          M x M matrix(e.g.: 32x32 double matrix)
%   Output:
%    cnet - Convolutional neural network with unchanged weights and biases
%    but with saved layers outputs (for the last example in inp)
%    out - network outputs (predictions), N x K matrix
%
% where N is the number of training examples
% M is the side length of one square image
% K is the number of output classes
%

numData = length(X);
out = nan(numData, cnet.numOutputs);

for nextDatum = 1:numData
